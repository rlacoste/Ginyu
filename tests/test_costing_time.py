...
def test_compute_cutting_time_ignores_pierce_count_in_the_formula():
    """Time is purely cut_length / feed_rate now -- pierce_count is still
    tracked as a geometric fact (reported per piece) but no longer costed
    ...
    material = MaterialParams(feed_rate_ipm=20.0)

    result = compute_cutting_time([piece], material, quantity=1)

    quote = result.pieces[0]
    expected_unit_time = 48.0 / 20.0
    assert quote.unit_time_min == pytest.approx(expected_unit_time)
    assert quote.pierce_count == 2
